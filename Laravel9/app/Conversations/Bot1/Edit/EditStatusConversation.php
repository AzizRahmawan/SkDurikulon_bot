...
class EditStatusConversation extends Conversation
{
    public $para;
    public $id_user;
    public $nik_lama;
    ...
    private function askNikPenduduk(){
        ...
        $this->ask($question, function(Answer $answer){
            if(Penduduk::where('nik',$answer->getText ())->exists() ){
                ...
                if (DB::table('edit_status_penduduk')->where('nik_lama',$answer->getText())->where('status_acc','belum_disetujui')->exists()){
                    $this->say('Permintaan Perubahan Data Pada Nik ini Belum Disetujui');
                } elseif (DB::table('edit_status_penduduk')->where('id_user2', $this->bot->getUser()->getId())->where('status_acc', 'disetujui')->where('nik_lama',$answer->getText())->exists()) {
                    $this->bot->reply('Permintaaan Edit Data Sebelumnya Telah Disetujui!!');
                    DB::table('edit_status_penduduk')->where('id_user2', $this->bot->getUser()->getId())->where('status_acc', 'disetujui')->where('nik_lama',$answer->getText())->delete();
                    $this->insertNikStatus();
                    $this->askStatusEditPenduduk();
                }elseif (DB::table('edit_status_penduduk')->where('id_user2', $this->bot->getUser()->getId())->where('status_acc', 'ditolak')->where('nik_lama',$answer->getText())->exists()){
                    $this->bot->reply('Permintaaan Edit Data Sebelumnya Telah Ditolak!!');
                    DB::table('edit_status_penduduk')->where('id_user', $this->bot->getUser()->getId())->where('status_acc', 'ditolak')->where('nik_lama',$answer->getText())->delete();
                    $this->insertNikStatus();
                    $this->askStatusEditPenduduk();
                }
                else{
                    if (DB::table('edit_status_penduduk')->where('id_user', $this->bot->getUser()->getId())->where('status_acc', 'belum_disetujui')->exists()){
    ...
    private function insertNikStatus(){
        ...
        foreach($dataPenduduk as $p){
            ...
            $message .= "Status Pernikahan  : " . $p->status . PHP_EOL;
        }
