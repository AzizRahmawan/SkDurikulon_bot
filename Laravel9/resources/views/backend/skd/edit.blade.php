...
@section('actskd')
    <li class="nav-item active"><a class="nav-link " href="{{ route('skd.index') }}">
@endsection
...
<div class="container-fluid">
    <h1 class="h3 mb-2 text-gray-800">Edit SK Domisili</h1>
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Edit SK Domisili</h6>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ isset($skd) ? route('skd.update',$skd->id_skd) : route('skd.store') }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="form-group">
                    <label for="nama">NAMA</label>
                    <input type="Text" class="form-control" name="nama" required value="{{ isset($skd) ? $skd->nama : '' }}">
                </div>

                <div class="form-group">
                    <label for="nik">NIK</label>
                    <input type="Text" class="form-control" name="nik" required value="{{ isset($skd) ? $skd->nik : '' }}">
                </div>
                <div class="form-group">
                    <label for="no_kk">No KK</label>
                    <input type="text" class="form-control" name="no_kk" required value="{{ isset($skd) ? $skd->no_kk : '' }}">
                </div>
                <div class="form-group">
                    <label for="tgl_lahir">Tempat dan Tanggal Lahir</label>
                    <input type="text" class="form-control" name="tgl_lahir" required value="{{ isset($skd) ? $skd->tmpt_tgl_lahir : '' }}">
                </div>
                <div class="form-group">
                    <label for="jk">Jenis Kelamin</label>
                    <input type="text" class="form-control" name="jk" required value="{{ isset($skd) ? $skd->jk : '' }}">
                </div>
                <div class="form-group">
                    <label for="pekerjaan">Pekerjaan</label>
                    <input type="text" class="form-control" name="pekerjaan" required value="{{ isset($skd) ? $skd->pekerjaan: '' }}">
                </div>
                <div class="form-group">
                    <label for="status">Status Pernikahan</label>
                    <input type="text" class="form-control" name="status" required value="{{ isset($skd) ? $skd->status: '' }}">
                </div>
                <div class="form-group">
                    <label for="agama">Agama</label>
                    <input type="text" class="form-control" name="agama" required value="{{ isset($skd) ? $skd->agama: '' }}">
                </div>
                <div class="form-group">
                    <label for="alamat">Alamat Pada KTP</label>
                    <textarea name="alamat_ktp" rows="5" class="form-control" required>{{ isset($skd) ? $skd->alamat_ktp : '' }}</textarea>
                </div>
                <div class="form-group">
                    <label for="alamat">Alamat/Tempat Tinggal Sekarang</label>
                    <textarea name="alamat_skr" rows="5" class="form-control" required placeholder="Orang tersebut di atas pada saat ini benar–benar Berdomisili / Bertempat Tinggal di?">{{ isset($skd) ? $skd->alamat_skr : '' }}</textarea>
                </div>
